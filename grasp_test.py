...
def detect_grasps(q_img, ang_img, threshold, width_img=None, no_grasps=5):
    is_peak = peak_local_max(q_img, min_distance=40, threshold_abs=threshold, indices=False)
...
def main():
  ...
  depth_img = image.DepthImage.from_tiff('imagenes/depth3.tiff')
  depth_img = np.asarray(depth_img)
  rgb_img = image.Image.from_file('imagenes/rgb3.jpg')
  ...
  MODEL_PATH="/agnostic_segmentation/models/FAT_trained_Ml2R_bin_fine_tuned.pth"
  ...
  print(roi)
  rgb_img=rgb_img[roi[1]:roi[3], roi[0]:roi[2]]
  ...
  grasps = detect_grasps(points_out, ang_out, 0.99, width_img=width_out, no_grasps=1)
  for g in grasps:
      print(g)
  print("grasps0: ", grasps[0])

  fig = plt.figure(figsize=(10, 10))
  ax = fig.add_subplot(2, 2, 1)
  depth_img=draw_angled_rec(grasps[0]["pix"][1], grasps[0]["pix"][0], grasps[0]["width"], grasps[0]["width"]/2, grasps[0]["ang"], color=(0.1,0.1,0), img=depth_img)

  ax.imshow(depth_img, cmap='gray')
  ax.set_title('Depth')
  ax.axis('off')
  points_out=draw_angled_rec(grasps[0]["pix"][1], grasps[0]["pix"][0], grasps[0]["width"], grasps[0]["width"]/2, grasps[0]["ang"], color=(0.1,0.1,0), img=points_out)
